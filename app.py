...
import sqlite3, os, json, io, csv, zipfile, urllib.request, urllib.error
...
FROM_EMAIL = 'TrueCare Hiring <hiring@equitycoresolutions.com>'
CC_EMAIL = 'hiring@equitycoresolutions.com'
...
def import_csv():
    """
    Import from a Google Sheet CSV export.
    Accepts column names: Name, Phone Number, Email 1, Email 2,
    Notes, NPN #, Interview Time  (case-insensitive, extra columns ignored).
    Adds new applicants WITHOUT deleting existing ones.
    """
    try:
        ...
        for row in reader:
            ...
            email      = pick(row, "email 1", "email address", "email")
            work_email = pick(row, "email 2", "work email")
            notes      = pick(row, "notes", "resume / background notes", "resume/background notes")
            npn        = pick(row, "npn #", "npn", "national producer number")
            interview  = pick(row, "interview time", "interview date")

            # Store NPN in chester_notes field (True Care's Notes) so it's visible
            chester_notes = f"NPN #: {npn}" if npn else ""

            conn.execute(
                """INSERT INTO applicants
                   (name, phone, email, work_email, interview_time,
                    interview_status, hired_status,
                    justin_notes, chester_notes,
                    start_date, resume_text, source, position, state, updated_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (name, phone, email, work_email, interview,
...
"""
    ...
    try:
        payload = json.dumps({
            ...
            "to": [target_email],
            "cc": [CC_EMAIL],
            "subject": f"TCA Onboarding Docs - {agent_name} - Signature requested by Andrew Cabrera",
            "html": html_body
        }).encode('utf-8')
